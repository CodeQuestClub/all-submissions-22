    ...
    for y in range(len(map_data)):
        for x in range(len(map_data[0])):
            ...
            if map_data[y][x] in "RBYG":
                ...
                # if "RBYG".index(map_data[y][x]) != my_index:
                #     enemies[map_data[y][x]] = (x, y)
            if map_data[y][x] == "Z":
                if not any([abs(hill[0]-x)<=1 and abs(hill[1]-y)<=1 for hill in hills]):
                    hills.append((x, y))
                                

    # Read map is called after read_index
    startpoint = spawns[my_index]
    manager.index = my_index
    manager.startpoint = spawns[my_index]
    manager.enemies = [spawns[my_index] for spawn in spawns if spawn != manager.startpoint]
    manager.objectives = [startpoint]
    manager.hills = hills
...
def handle_events(events):
    ...
    requests = manager.run(events, closest_site)
