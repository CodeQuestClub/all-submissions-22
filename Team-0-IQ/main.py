...
from codequest22.server.events import DepositEvent, DieEvent, ProductionEvent, ZoneActiveEvent, FoodTileActiveEvent
...
def handle_events(events):
    ...
    spawned_this_tick = 0
    while(
        total_ants < (stats.general.MAX_ANTS_PER_PLAYER // 3) and
    ...
    while (
        ...
        if my_energy > 90:
            spawned_this_tick += 1
            total_ants += 1
            requests.append(SpawnRequest(AntTypes.FIGHTER, id=None, color=None, goal=spawns[1]))
            my_energy -= stats.ants.Fighter.COST

            spawned_this_tick += 1
            total_ants += 1
            requests.append(SpawnRequest(AntTypes.FIGHTER, id=None, color=None, goal=spawns[2]))
            my_energy -= stats.ants.Fighter.COST

            spawned_this_tick += 1
            total_ants += 1
            requests.append(SpawnRequest(AntTypes.FIGHTER, id=None, color=None, goal=spawns[3]))
            my_energy -= stats.ants.Fighter.COST
        else:
            break
    return requests
