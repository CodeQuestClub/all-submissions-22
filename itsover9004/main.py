...
def read_map(md, energy_info):
    global map_data, spawns, food, distance, closest_site, second_closest, hill, fighter_target
    map_data = md
    fighter_target = my_index
...
def handle_events(events):
    global my_energy, total_ants, hill_id, spawn_settlers, fighter_target
    requests = []

    if my_index == 0:
        fighter_target = 1
    else:
        fighter_target = my_index - 1
    ...
    if my_energy > 500:
        spawned_this_tick += 1
        total_ants += 1
        requests.append(SpawnRequest(AntTypes.FIGHTER, id=None, color=None, goal=spawns[fighter_target]))
    ...
    while (
        ...
        my_energy >= stats.ants.Worker.COST*2 and
        spawn_settlers == None 
    ):
        spawned_this_tick += 1
        total_ants += 1
        ...
        spawned_this_tick += 1
        total_ants += 1
        requests.append(SpawnRequest(AntTypes.WORKER, id=None, color=None, goal=second_closest))
        my_energy -= stats.ants.Worker.COST

    while (
        total_ants < 80 and
        spawned_this_tick < stats.general.MAX_SPAWNS_PER_TICK and
        my_energy >= stats.ants.Worker.COST + stats.ants.Settler.COST and
        spawn_settlers == True
    ):
        spawned_this_tick += 1
        total_ants += 1
        requests.append(SpawnRequest(AntTypes.SETTLER, id=None, color=None, goal=hill[hill_id]))
